...
class AutoGraderApp:
    def __init__(self, root):
        ...
        self.plagiarism_radio = tk.Radiobutton(self.grading_method_frame, text="Plagiarism Detection (Web search + AI paraphrase check)", 
                                               variable=self.grading_method, value="plagiarism", font=("Arial", 9), 
                                               command=self.update_grading_inputs)
        self.plagiarism_radio.pack(anchor=tk.W, padx=20, pady=2)
        ...
        self.plagiarism_frame = tk.Frame(root)
        
        tk.Label(self.plagiarism_frame, text="Assignment Topic:", font=("Arial", 9, "bold")).pack(anchor=tk.W, padx=5)
        tk.Label(self.plagiarism_frame, text="Enter the topic of the assignment (e.g., 'photosynthesis in plants', 'American Revolution'):", font=("Arial", 8), fg="gray").pack(anchor=tk.W, padx=5)
        
        self.plagiarism_topic_input = tk.Entry(self.plagiarism_frame, width=60, font=("Arial", 10))
        self.plagiarism_topic_input.pack(pady=5, padx=5, fill=tk.X)
        
        self.ai_paraphrase_enabled = tk.BooleanVar(value=False)
        self.ai_paraphrase_checkbox = tk.Checkbutton(self.plagiarism_frame, text="Enable AI paraphrase detection",
                                                      variable=self.ai_paraphrase_enabled, font=("Arial", 9),
                                                      command=self.toggle_ai_paraphrase_warning)
        self.ai_paraphrase_checkbox.pack(anchor=tk.W, padx=5, pady=5)
        
        self.ai_paraphrase_warning = tk.Label(self.plagiarism_frame, 
                                               text="This feature can make mistakes. Always review flagged content manually before taking action!",
                                               fg="#FF6B6B", font=("Arial", 8, "bold"), wraplength=400)
        
        self.plagiarism_button = tk.Button(self.plagiarism_frame, text="Search the Web for Plagiarism", 
                                           command=self.detect_plagiarism, bg="white", fg="black", font=("Arial", 10, "bold"))
        self.plagiarism_button.pack(pady=10)
    ...
    def update_grading_inputs(self):
        ...
        self.plagiarism_frame.pack_forget()
        
        method = self.grading_method.get()
        if method == "ai":
            self.ai_notes_frame.pack(pady=5, padx=20, fill=tk.X, before=self.grade_button)
        elif method == "plagiarism":
            self.plagiarism_frame.pack(pady=5, padx=20, fill=tk.X, before=self.grade_button)
        else:
            self.keyword_input_frame.pack(pady=5, padx=20, fill=tk.X, before=self.grade_button)

    def toggle_ai_paraphrase_warning(self):
        if self.ai_paraphrase_enabled.get():
            self.ai_paraphrase_warning.pack(anchor=tk.W, padx=5, pady=2, before=self.plagiarism_button)
        else:
            self.ai_paraphrase_warning.pack_forget()
...
"""
    ...
    def detect_plagiarism(self):
        student_text = self.text_input.get("1.0", tk.END).strip()
        if not student_text:
            messagebox.showwarning("Warning", "Please upload or type the student's assignment text first.")
            return
        
        topic = self.plagiarism_topic_input.get().strip()
        if not topic:
            messagebox.showwarning("Warning", "Please enter the assignment topic.")
            return
        
        self.training_status_label.config(text="Searching for plagiarism...", fg="blue")
        self.plagiarism_button.config(state=tk.DISABLED)
        self.root.update_idletasks()
        
        thread = threading.Thread(target=self.detect_plagiarism_thread, args=(student_text, topic))
        thread.daemon = True
        thread.start()
    
    def detect_plagiarism_thread(self, student_text, topic):
        try:
            common_words = {
                ...
                'when', 'whenever', 'where', 'wherever', 'whether', 'which', 'who',
                'whom', 'whose', 'what', 'whatever', 'whichever', 'whoever', 'whomever',
            ...
            words = re.findall(r'\b[a-zA-Z]{4,}\b', student_text.lower())
            key_phrases = [w for w in words if w not in common_words]
            
            from collections import Counter
            word_counts = Counter(key_phrases)
            ...
            for sent in sentences:
                ...
                if len(sent) > 30 and len(sent) < 200:
                    ...
                    if len(sent_words - common_words) >= 3:
            ...
            try:
                ...
                for sentence in key_sentences[:3]:
                    search_words = ' '.join(sentence.split()[:8])
                    results = ddgs.text(f'"{search_words}"', max_results=2)
                    for r in results:
                        web_matches.append({
                ...
                self.root.after(0, lambda: self.training_status_label.config(text=f"Web search error: {str(e)[:50]}", fg="red"))
            
            if self.ai_paraphrase_enabled.get() and self.smollm_model is not None and self.smollm_tokenizer is not None:
                try:
                    for sentence in key_sentences[:3]:
                        prompt = f"""Analyze this sentence from a student essay about "{topic}":

"{sentence}"

Does this sentence appear to be paraphrased from common sources without citation? 
Consider: unusual phrasing, overly sophisticated vocabulary for a student, or text that sounds like it was reworded from an original source.
Answer with YES or NO, then briefly explain why."""

                        inputs = self.smollm_tokenizer(prompt, return_tensors="pt", truncation=True, max_length=512)
                        outputs = self.smollm_model.generate(
                            inputs.input_ids,
                            max_new_tokens=100,
                            temperature=0.7,
                        ...
                        response = self.smollm_tokenizer.decode(outputs[0], skip_special_tokens=True)
                        
                        answer_part = response[len(prompt):].strip() if len(response) > len(prompt) else response
                        
                        if 'yes' in answer_part.lower()[:50]:
                            paraphrase_suspects.append({
                                'sentence': sentence,
                                'ai_analysis': answer_part[:200]
                            })
                except Exception as e:
                    pass
            ...
            result = {
                'topic': topic,
                'student_text': student_text,
                'keywords_checked': top_keywords,
                'sentences_checked': key_sentences,
                'web_matches': web_matches,
                'paraphrase_suspects': paraphrase_suspects
            }
            
            self.root.after(0, self.show_plagiarism_results, result)
            
        except Exception as e:
            self.root.after(0, lambda: messagebox.showerror("Plagiarism Check Error", f"Error during plagiarism check: {e}"))
            self.root.after(0, self.finish_plagiarism_check)
    
    def finish_plagiarism_check(self):
        self.training_status_label.config(text="")
        self.plagiarism_button.config(state=tk.NORMAL)
    
    def show_plagiarism_results(self, result):
        self.finish_plagiarism_check()
        
        unique_urls = set()
        unique_matches = []
        for match in result['web_matches']:
            if match['url'] not in unique_urls:
                unique_urls.add(match['url'])
                unique_matches.append(match)
        
        num_matches = len(unique_matches)
        num_paraphrase = len(result['paraphrase_suspects'])
        
        if num_matches >= 10 or num_paraphrase >= 2:
            risk_level = "HIGH"
            risk_color = "red"
        elif num_matches >= 5 or num_paraphrase >= 1:
            risk_level = "MEDIUM"
            risk_color = "orange"
        elif num_matches > 0:
        ...
        summary = f"Plagiarism Risk: {risk_level}\n"
        summary += f"Web matches found: {num_matches}\n"
        summary += f"Potential paraphrased content: {num_paraphrase}"
        
        messagebox.showinfo("Plagiarism Check Complete", summary)
        
        self.show_plagiarism_report(result, unique_matches, risk_level)
    
    def show_plagiarism_report(self, result, unique_matches, risk_level):
        import datetime
        
        report_window = tk.Toplevel(self.root)
        report_window.title("Plagiarism Detection Report")
        report_window.geometry("700x600")
        
        main_frame = tk.Frame(report_window)
        main_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        
        tk.Label(main_frame, text="PLAGIARISM DETECTION REPORT", font=("Arial", 14, "bold")).pack(pady=5)
        tk.Label(main_frame, text=f"Risk Level: {risk_level}", font=("Arial", 12, "bold"), 
                 fg="green" if risk_level == "LOW" else ("orange" if "MEDIUM" in risk_level else "red")).pack(pady=5)
        
        report_text = scrolledtext.ScrolledText(main_frame, height=25, width=80, font=("Courier", 9))
        report_text.pack(pady=10, fill=tk.BOTH, expand=True)
        
        report_content = f"""PLAGIARISM DETECTION REPORT
{'='*60}
...
KEYWORDS ANALYZED
{'='*60}
{', '.join(result['keywords_checked'])}

{'='*60}
WEB SEARCH RESULTS ({len(unique_matches)} unique sources found)
{'='*60}
"""
        
        for i, match in enumerate(unique_matches[:15], 1):
            report_content += f"""
{i}. {match['title'][:80]}
   URL: {match['url']}
...
"""
            if 'matched_sentence' in match:
                report_content += f"   MATCHED SENTENCE: {match['matched_sentence'][:100]}...\n"
        
        if result['paraphrase_suspects']:
            report_content += f"""
{'='*60}
POTENTIAL PARAPHRASED CONTENT (AI Analysis)
{'='*60}
"""
            for i, suspect in enumerate(result['paraphrase_suspects'], 1):
                report_content += f"""
{i}. Suspicious sentence:
   "{suspect['sentence'][:150]}..."
   
   AI Analysis: {suspect['ai_analysis'][:200]}
"""
...
DISCLAIMER
{'='*60}
This plagiarism check is not definitive. Web search results may include
legitimate sources that the student properly cited, or common knowledge.
The AI paraphrase detection is experimental and may produce false positives.
Always review flagged content manually before taking action.
...
"""
        ...
        def download_txt():
            file_path = filedialog.asksaveasfilename(
                defaultextension=".txt",
                filetypes=[("Text Files", "*.txt"), ("All Files", "*.*")],
                initialfile=f"Plagiarism_Report_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
            )
            if file_path:
                try:
                    with open(file_path, 'w') as f:
        ...
        tk.Button(button_frame, text="Download Report", command=download_txt, bg="white", fg="black", font=("Arial", 10, "bold")).pack(side=tk.LEFT, padx=5)
        tk.Button(button_frame, text="Close", command=report_window.destroy, bg="white", fg="black", font=("Arial", 10, "bold")).pack(side=tk.LEFT, padx=5)
