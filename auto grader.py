...
class AutoGraderApp:
    def __init__(self, root):
        ...
        self.ai_radio = tk.Radiobutton(self.grading_method_frame, text="AI-Assisted Grading (Qwen2.5 - Quality assessment)", 
        ...
        self.visualize_button = tk.Button(root, text="Visualize Scores", command=self.visualize_scores, bg="white", fg="black")
        ...
        # Load Qwen2.5 model on startup in background
        self.load_qwen_on_startup()
    ...
    def visualize_scores(self):
        ...
        plt.xlabel("Points")
        plt.title("Assessment Scores by Criterion")
        plt.tight_layout()
        plt.show()
    ...
    def load_qwen_on_startup(self):
        """Load Qwen2.5 model in background thread on startup"""
        thread = threading.Thread(target=self.load_qwen_model_thread)
        thread.daemon = True
        thread.start()
    
    def load_qwen_model_thread(self):
        """Thread worker to load Qwen2.5 model"""
        try:
            ...
            self.root.after(0, lambda: self.training_status_label.config(text="Loading Qwen2.5 model...", fg="blue"))
            ...
            self.root.after(0, lambda: self.training_status_label.config(text="Qwen2.5 model loaded successfully!", fg="green"))
        except Exception:
            self.root.after(0, lambda: messagebox.showerror("Model Load Error", f"Failed to load Qwen2.5 model: {e}"))
            self.root.after(0, lambda: self.training_status_label.config(text="Failed to load Qwen2.5 model", fg="red"))
    ...
    def load_qwen_model(self):
        ...
        try:
            ...
            self.training_status_label.config(text="Loading Qwen2.5 model...", fg="blue")
            self.root.update_idletasks()
            ...
            self.training_status_label.config(text="Qwen2.5 model loaded successfully!", fg="green")
        except Exception as e:
            messagebox.showerror("Model Load Error", f"Failed to load Qwen2.5 model: {e}")
            self.training_status_label.config(text="Failed to load Qwen2.5 model", fg="red")
    ...
    def ai_assisted_grading(self, student_text, ai_instructions):
        ...
        if self.smollm_model is None or self.smollm_tokenizer is None:
            messagebox.showerror("Error", "Qwen2.5 model not loaded. Cannot perform AI grading.")
            return None
...
Based on the student's response and the grading instructions above, provide a score and justification.
