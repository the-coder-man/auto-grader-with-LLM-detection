...
# Suppress DeprecationWarning from spaCy
warnings.filterwarnings("ignore", category=UserWarning)
...
class AutoGraderApp(tk.Tk):
    ...
    def create_analyze_tab(self):
        """Creates the main analysis tab."""
        analyze_tab = tk.Frame(self.notebook, bg="#f4f7f9")
        self.notebook.add(analyze_tab, text="Analyze & Grade")

        main_frame = tk.Frame(analyze_tab, bg="#f4f7f9", padx=15, pady=15)
    ...
    def create_train_tab(self):
        """Creates the training tab for the GUI."""
        self.train_tab = tk.Frame(self.notebook, bg="#f4f7f9")
        self.notebook.add(self.train_tab, text="Train")
        
        # Training Data Frame
        train_frame = tk.Frame(self.train_tab, bg="#ffffff", padx=15, pady=15, bd=2, relief="groove")
        train_frame.pack(fill="both", expand=True, padx=10, pady=10)
        
        # Header
        tk.Label(train_frame, text="Model Training", font=("Helvetica", 14, "bold"), bg="#ffffff").pack(pady=(0, 10))
        
        # Description
        tk.Label(train_frame, text="Here you can train the LLM detection model by providing human-written and AI-generated text files.", bg="#ffffff").pack(pady=(0, 10))
        
        # Upload and Label Frame
        upload_frame = tk.Frame(train_frame, bg="#ffffff")
        upload_frame.pack(pady=10)
        
        self.label_choice = tk.StringVar(self)
        self.label_choice.set("Human-written")
        
        tk.Radiobutton(upload_frame, text="Human-written", variable=self.label_choice, value="Human-written", bg="#ffffff").pack(side=tk.LEFT, padx=5)
        tk.Radiobutton(upload_frame, text="AI-generated", variable=self.label_choice, value="AI-generated", bg="#ffffff").pack(side=tk.LEFT, padx=5)
        
        tk.Button(upload_frame, text="Upload Files", command=self.upload_and_label_files, bg="gray", fg="white").pack(side=tk.LEFT, padx=5)
        
        # Retrain button frame
        retrain_frame = tk.Frame(train_frame, bg="#ffffff")
        retrain_frame.pack(pady=10)
        
        # Corrected line
        tk.Button(retrain_frame, text="Retrain Model", command=self.retrain_model, bg="purple", fg="white").pack(padx=5, pady=5)
        
        # Training Status Label
        self.training_status_label = tk.Label(train_frame, text="", bg="#ffffff", fg="green")
        self.training_status_label.pack(pady=10)
        
    def upload_and_label_files(self):
        """Prompts the user to select multiple files and saves them to the DB with the selected label."""
        file_paths = filedialog.askopenfilenames(title="Select Training Files", filetypes=[("Text Files", "*.txt"), ("PDF Files", "*.pdf")])
        if not file_paths:
            return
        
        label = 0 if self.label_choice.get() == "Human-written" else 1
        ...
        for file_path in file_paths:
            try:
                ...
                messagebox.showerror("File Error", f"Could not process {os.path.basename(file_path)}: {e}")
                
        self.training_status_label.config(text=f"Successfully added {len(file_paths)} files to training data.")

    def retrain_model(self):
        """Triggers the model retraining process."""
        self.training_status_label.config(text="Training model...", fg="orange")
        self.update_idletasks()
        
        status_message = train_model()
        self.training_status_label.config(text=status_message)
        if "successfully" in status_message.lower():
            self.training_status_label.config(fg="green")
        else:
            self.training_status_label.config(fg="red")
